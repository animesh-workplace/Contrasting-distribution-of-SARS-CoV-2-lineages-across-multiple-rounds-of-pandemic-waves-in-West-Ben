...
parser.add_argument(
    "--type",
    help="Enter division or location, where division is state and location is district",
)
...
month_wise_mutation_count = pandas.DataFrame(
    index=list(voc_to_track.keys()), columns=month_keys
)
month_wise_mutation_percent = pandas.DataFrame(
    index=list(voc_to_track.keys()), columns=month_keys
)
month_wise_mutation_combined = pandas.DataFrame(
    index=list(voc_to_track.keys()), columns=month_keys
)
...
for voc_type, entries in voc_to_track.items():
    ...
    for i in entries:
        if "pangolin" in list(i.keys()):
            voc_metadata = pandas.concat(
                [voc_metadata, metadata.loc[metadata["lineage"].isin(i["pangolin"])]]
            )
        if "nextstrain" in list(i.keys()):
            voc_metadata = pandas.concat(
                [voc_metadata, metadata.loc[metadata["clade"].isin(i["nextstrain"])]]
            )
    ...
    for i in month_keys:
        voc_metadata_monthly = voc_metadata.iloc[
            voc_metadata.index[voc_metadata["collection_month"] == i].tolist()
        ]
        for index, k in dict(
            collections.Counter(voc_metadata_monthly[type_of_table])
        ).items():
            final_df_month_count.loc[index][i] = k
            final_df_month_percent.loc[index][i] = round(
                (k / district_month_wise_sampling.loc[index, i]) * 100, 2
            )
            final_df_month_combined.loc[index][
                i
            ] = f"{final_df_month_count.loc[index][i]} ({final_df_month_percent.loc[index][i]}%)"
...
for i in month_keys:
    month_wise_mutation_percent[i] = round(
        (
            pandas.to_numeric(month_wise_mutation_count[i])
            / len(metadata.index[metadata["collection_month"] == i].tolist())
        )
        * 100,
        2,
    )
